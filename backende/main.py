...
async def update_session_heartbeat(
    ...
    if db.update_session_heartbeat(session_id):
        active_sessions = db.get_user_active_sessions(current_user.email)
        db.update_user_sessions(current_user.email, active_sessions)
        return {"status": "ok"}
